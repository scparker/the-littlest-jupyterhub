...
def ensure_jupyterhub_service(prefix):
    ...
    if not os.path.exists(proxy_secret_path):
        with open(proxy_secret_path, 'w') as f:
            f.write('CONFIGPROXY_AUTH_TOKEN=' + '0123456789abcdef0123456789abcdef')
...
def ensure_user_environment(user_requirements_txt_file):
    ...
    miniconda_installer_md5 = "a01cbe45755d576c2bb9833859cf9fd7"
...
def ensure_symlinks(prefix):
    ...
    print(tljh_config_src)
    print(tljh_config_dest)
    print("\n")
    ...
    if os.path.exists(tljh_config_dest):
        if os.path.realpath(tljh_config_dest) != tljh_config_src:
            ...
            raise FileExistsError('tljh-config exists but is not a symlink to tljh_config_src')
        else:
